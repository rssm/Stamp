...
class ARStamp: SCNReferenceNode {
    ...
    private var recentARStampDistances = [Float]()
    ...
    func reset() {
        recentARStampDistances.removeAll()
    }
    ...
    func setTransform(_ newTransform: float4x4,
        ...
        if smoothMovement {
            ...
            recentARStampDistances.append(hitTestResultDistance)
            recentARStampDistances = Array(recentARStampDistances.suffix(10))
            
            let averageDistance = recentARStampDistances.average!
...
extension ARStamp {
    ...
    static let availableObjects: [ARStamp] = {
        ...
        return fileEnumerator.compactMap { element in
            ...
            return ARStamp(url: url)
        }
    }()
    
    /// Returns a `VirtualObject` if one exists as an ancestor to the provided node.
    static func existingObjectContainingNode(_ node: SCNNode) -> ARStamp? {
        if let virtualObjectRoot = node as? ARStamp {
